...
if (isset($_POST["account"])){
    ...
    //找尋該會員資料
    $query_RecFindUser = "SELECT CustomerUsername, CustomerMail FROM Customer WHERE CustomerUsername='{$muser}'";
    $RecFindUser = $db_link->query($query_RecFindUser);
    if($RecFindUser->num_rows==0){
        header("Location: customer_passmail.php?errMsg=1&account={$muser}");
    }else{
        //取出帳號密碼的值
...
<input name="account" type="text" class="logintextbox" id="mail"></p>
<p align="center">
...
<p align="right"><a href="customer_join.php">加入會員</a></p></div>
